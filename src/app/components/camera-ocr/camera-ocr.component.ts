...
export class CameraOCRComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for (let index = 0; index < 256 * 256 * 256 - 1; index++) {
      const rgb = this.maßOfRGBService.getMaßRGB_opposed(index);
  ...
  triggerImage(imageData: any) {
    this.netSerive.callNetLoop(imageData);
  }
  ...
  //   constructor(private maßOfRGBService: MaßOfRGBService, private clusterService: ClusterService) { }
  ...
  //     this.clusterService.getKMeansCluster(20,imageData,0,255);
